...
class SubscribersFeed extends XmlFeed
{
    ...
    public function generate($what = null): int
    {
        ...
        $gate='http://'.$this->_user->username.'/api/?gate=clients/getNewsletterEmailShops/175/soap/wsdl&lang=pol';
        ...
        if($what == 'objects') {
            ...
            return $this->createSubscriberObjects(); 
    ...
    private function checkQueueConstraints(){ // todo
        ...
        $response = $this->_client->getNewsletterEmailShops($request->getRequest());
        // var_dump($response);
    ...
    private function createSubscriberObjects()
    {
        echo "function createSubscriberObjects ".PHP_EOL;
        if (IntegrationData::getData('INITIAL_SUBSCRIBERS_DONE', $this->_user->id)) {
                $this->_request->addParam('date', [
                        'from'=> IntegrationData::getDataValue('LAST_SUBSCRIBER_INTEGRATION_DATE', $this->_user->id),
        ...
        try {
            ...
            if ($this->_queue->page >= $this->_queue->max_page) {
                IntegrationData::setData('LAST_SUBSCRIBER_INTEGRATION_DATE', date('Y-m-d'), $this->_user->id);
                IntegrationData::setData('INITIAL_SUBSCRIBERS_DONE', 1, $this->_user->id);
                echo "finished";
                // die ("!!FIN!");
                return 10;
            }
            ...
            $request=$this->_request;
            $request->addParam('results_page', $this->_queue->page);  
            $response = $this->_client->getNewsletterEmailShops($request->getRequest());
            ...
            foreach ($response->clients as $customer) {
            	if ($customer->registered!='n'){
                    echo "alreadyregistered ".$customer->registered." ";
            		continue;
            	}
                ...
                $customer_item['customer_id'] = 'popup-'.htmlspecialchars($customer->email);
                $customer_item['email'] = htmlspecialchars($customer->email);
                // echo $this->getCorrectDbDate($customer->shops[0]->date_modification).PHP_EOL;
                $customer_item['registration'] = $this->getCorrectDbDate($customer->shops[0]->date_modification);
                $customer_item['last_modification_date'] = $this->getCorrectDbDate($customer->shops[0]->date_modification);
                $customer_item['newsletter_frequency'] = $customer->shops[0]->approval=='y'?'every day':'never';
                $customer_item['nlf_time'] = $this->getCorrectDbDate($customer->shops[0]->date_modification);
